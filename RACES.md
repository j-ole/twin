...
`test.sh` enables the race detector and a detected race is written to a
`twin-race-report.*` file in the repo root.
...
To tell your own race apart from one main already has, check whether the
commit that introduced the racing code is an ancestor of main.
...
If it already existed on main and doesn't block the current branch, file an
issue ticket.

Write a throwaway repro instead, one that reports the race on nearly every run,
confirm it goes clean with the fix, then delete it without committing. Don't
pause to have the repro reviewed, it isn't a test we are keeping.
